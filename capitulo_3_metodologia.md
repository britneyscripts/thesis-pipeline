...
4. **Delimitação da Janela do CrUX e Baseline de Laboratório**: A API do Chrome UX Report fornece dados de campo agregados em janela móvel de 28 dias ($p75$). Para e-commerces ou PDPs com volume de tráfego insuficiente para gerar dados de campo no CrUX, as métricas sintéticas de laboratório capturadas via PageSpeed Insights API servem como baseline de latência controlada.
...
2. **Taxa de Requisição Responsável (*Rate Limiting*)**: Os scripts de extração em Python aplicam intervalos de latência (*backoff / sleep = 1s*) entre requisições para evitar sobrecarga de servidores ou consumo indevido de banda dos e-commerces monitorados (`extract_content.py`).
...
- A decodificação dos agentes foi fixada em modo determinístico (`temperature = 0.0`, amostragem gananciosa) com instrução neutra de sistema declarada. Como a API do Vertex AI não expõe parâmetro de semente (`seed`) para os modelos Gemini 2.5, a variação residual entre execuções é tratada como fonte de variância no desenho longitudinal.
